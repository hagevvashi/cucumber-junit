...
function convertFeature(featureJson, options) {
    ...
    return elements
        ...
        .map(function (scenarioJson) {
            ...
            var a = scenario.testsuite[0];
            var b = scenario.testsuite[1];
            var c = scenario.testsuite.slice(2);
            var d = c.map(function (v) {
                var testcase = v.testcase;
                testcase[0]._attr.file = featureJson.uri;
                return { testcase: testcase };
            });

            scenario.testsuite = [a, b].concat(d);
            if (featureJson.uri) {
